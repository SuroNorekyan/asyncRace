...
function computeEtaMs(distance, velocity) {
...
export const startCar = (id) => async (dispatch, getState) => {
    clearTimers(id);
    ...
    try {
        ...
        const loop = () => {
            ...
            if (!state || state.status !== 'driving' || !state.startedAt || !state.etaMs)
                return;
            const now = Date.now();
...
export const stopCar = (id) => async (dispatch) => {
    clearTimers(id);
...
export const startRaceAll = () => async (dispatch, getState) => {
...
export const resetRaceAll = () => async (dispatch, getState) => {
    const cars = getState().garage.items;
...
export const bindWinnerListener = () => (dispatch, getState) => {
    const tick = () => {
        const st = getState();
        if (st.race.global !== 'inProgress')
            return;
